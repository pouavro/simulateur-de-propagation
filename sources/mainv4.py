...
nombre_de_personnages = 5  # Nombre de personnages à ajouter
...
img_bonhomme = img_bonhomme.subsample(img_bonhomme.width() // TAILLE_CASE)


# ────────────── Données ──────────────
bonhommes = {}  # dictionnaire : clé = coord, valeur = objet canvas
...
def starting_grid():
    ...
    bonhommes.clear() # On vide aussi le dictionnaire des bonhommes
    ...
    # --- ON AJOUTE LE PERSONNAGE ICI ---
    for i in range(nombre_de_personnages):
        x = random.randint(0, TAILLE_GRILLE - 1)
        y = random.randint(0, TAILLE_GRILLE - 1)
        bonhommes[(x, y)] = ajouter_bonhomme(x, y)
...
def verification_des_bonhommes():
    for coord, obj in bonhommes.items():
        lig, col = coord
        # vérifier si le bonhomme est sur une case infectée
        if coord in cases_infectees:
            # supprimer le bonhomme du canvas
            canvas.delete(obj)
            # supprimer le bonhomme du dictionnaire
            del bonhommes[coord]
